...
public class HinnakiriServiceImpl implements HinnakiriService {
	...
	public GetHinnakiriResponse getHinnakiri(java.lang.String parameters) 
			throws HinnakiriNumberFormatException, HinnakiriNegNumberFormatException, 
				HinnakiriNullHindException, HinnakiriNumOfDigitsException
	{
		...
		if (!IsikukoodValidator.isValid(parameters)) {
			// WebServiceException oskab SOAPFault exceptionit genereerida
		...
		*/
		...
		// null hinna kontroll
		if (maximumPrice == 0) {
			throw new HinnakiriNullHindException();
		}
		
		// komakohtade kontroll
		String[] splittedParameter = parameters.split("\\.");
		
		if (splittedParameter.length > 1) {
			if (splittedParameter[1].length() > 2) {
				throw new HinnakiriNumOfDigitsException();
			}
		}
		try {
